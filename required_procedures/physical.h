...
struct tabular_atmo {
  ...
  VecDoub nHlist, nCO2list, f2list, f1list, D, K, Hinv, H0inv, T, Tp, alt, logalt;
  double f3, f4, f5;
  double dalt;
  ...
  tabular_atmo(double nexoo, double Texoo, int n_interp_ptss=10000, string filename="") : 
    ...
    nHlist.resize(n_quad_pts);
    nCO2list.resize(n_quad_pts);
    f2list.resize(n_quad_pts);
    f1list.resize(n_quad_pts);
    D.resize(n_quad_pts);
    K.resize(n_quad_pts);
    Hinv.resize(n_quad_pts);
    H0inv.resize(n_quad_pts);
    T.resize(n_quad_pts);
    Tp.resize(n_quad_pts);

    logalt.resize(n_quad_pts);
    alt.resize(n_quad_pts);
    ...
    lognHinterp = Linear_interp(logaltvec, lognHvec); // interpolate
    lognCO2interp = Linear_interp(logaltvec, lognCO2vec); // interpolate
