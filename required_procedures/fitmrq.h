//fitmrq.h -- Levenberg-Marquardt chi-squared minimization and fitting routines
...
struct Fitmrq {
  ...
  Fitmrq(VecDoub_I &xx, VecDoub_I &yy, VecDoub_I &ssig,
	 VecDoub_I &aa,
	 void funks(const double, VecDoub_I &, double &, VecDoub_O &),
  ...
  void mrqcof(VecDoub_I &a, MatDoub_O &alpha, VecDoub_O &beta) {
    ...
    for (i = 0; i < ndat; i++) { // loop over all data
      ...
      funcs(x[i],a,ymod,dyda); //get the local fit value and derivs
